        ...
        //ORDER PAGE BEGINS
            ...
            $.get("partials/order.html", function (data) {
                ...
                $("#mySingleLineText" || "#myTextArea").on("change", function () {
                    ...
                    $("#changeEvent").html("Thank you, " + value +
                        ", for shopping with CellSpot.");
                });
                ...
                $("#myButton").on('click', function () {
                    ...
                    var displayOrder = function () {
                            ...
                            for (var loop = 0; loop < checkBoxValue.length; loop++) {
                                for (var loop2 in phonePrices) {
                                    if (phonePrices[loop2].name === checkBoxValue[loop]) {
                                        total.push(phonePrices[loop2].name + ": " + "$" +
                                            phonePrices[loop2].price + "<br>");
                            ...
                            for (var loop3 = 0; loop3 < radioValue.length; loop3++) {
                                for (var loop4 in phonePrices) {
                                    if (phonePrices[loop4].name === radioValue[loop3]) {
                                        total.push(phonePrices[loop4].name + ": " + "$" +
                                            phonePrices[loop4].price + "<br>");
                            ...
                            $("#summary").append(
                                "<div class='totalAmount'><span class='label label-pill label-success'>Total Amount</span> " +
                                "$" + calculate + "</div>");
                        } //displayorder


                    //if/else that verifies fields are entered
                    if ($("#myTextArea").val().length === 0 && $("#mySingleLineText").val().length !==
                        0) {
                    ...
                    } else if ($("#myTextArea").val().length !== 0 && $("#mySingleLineText").val()
                        .length === 0) {
            ...
            $.getJSON("jsonDatabase/finalPhones.json", function (data) {
                ...
                $.each(data, function (index, item) {
                        html += '<div class="col-xs-12 col-lg-4">' +
                            ...
                            '<div class="phoneBrand"><strong>Brand </strong>' + item.brand +
                            '</div>' +
                            '<div class="phoneStorage"><strong>Memory </strong>' + item.storage +
                            '</div>' +
